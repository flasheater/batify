...
$(BAT_UDEV):$(SRC)
	...
	@echo -e 'RUN+="/usr/local/bin/batify.sh %k $$attr{capacity} none"' >> $(BAT_UDEV)
...
$(PWR_UDEV):$(SRC)
	...
	@echo -e 'RUN+="/usr/local/bin/batify.sh none none 0"' >> $(PWR_UDEV)
	@echo >> $(PWR_UDEV)
	@echo -e 'SUBSYSTEM=="power_supply", ACTION=="change", \\' >> $(PWR_UDEV)
	@echo -e 'ENV{POWER_SUPPLY_ONLINE}=="1", ENV{POWER}="on", \\' >> $(PWR_UDEV)
	@echo -e 'OPTIONS+="last_rule", \\' >> $(PWR_UDEV)
	@echo -e 'RUN+="/usr/local/bin/batify.sh none none 1"' >> $(PWR_UDEV)
